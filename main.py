    ...
    try:
        ...
        if data.get('ret') == 1:
            print_with_time(f"签到成功: {data.get('msg', '获得奖励')}", "SUCCESS")
             # 进行推送
            if SCKEY != '':
                    push_url = 'https://sctapi.ftqq.com/{}.send?title=机场签到&desp={}'.format(SCKEY, f"签到成功: {data.get('msg', '获得奖励')}")
            ...
            # 进行推送
            if SCKEY != '':
                    push_url = 'https://sctapi.ftqq.com/{}.send?title=机场签到&desp={}'.format(SCKEY, f"今日已签到: {data.get('msg', '请明天再来')}")
                    requests.post(url=push_url)
                    print('推送成功')          
            return True
        else:
            ...
            # 进行推送
            if SCKEY != '':
                    push_url = 'https://sctapi.ftqq.com/{}.send?title=机场签到&desp={}'.format(SCKEY, f"签到失败: {data.get('msg', '未知错误')}")
                    requests.post(url=push_url)
                    print('推送成功')      
            return False
